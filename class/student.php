...
class Student
{
    ...
    public function create_student_info($post_data=array()){
       ...
       $image= mysqli_real_escape_string($this->conn,trim($post_data['image']));

       $sql="INSERT INTO students (student_name, email_address, contact,country,gender) VALUES ('$student_name', '$email_address', '$contact','$country','img''$gender')";
    ...
    public function update_student_info($post_data=array()){
       ...
       $contact= mysqli_real_escape_string($this->conn,trim($post_data['contact']));
       $country= mysqli_real_escape_string($this->conn,trim($post_data['country']));
       $country= mysqli_real_escape_string($this->conn,trim($post_data['image']));
       $student_id= mysqli_real_escape_string($this->conn,trim($post_data['id']));

       $sql="UPDATE students SET student_name='$student_name',email_address='$email_address',contact='$contact',country='$country',image='$image',gender='$gender' WHERE student_id =$student_id";
